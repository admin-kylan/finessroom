    ...
    methods:{
      ...
      singleDeleteUsingItem:function (item,index) {
        ...
        savePackage:function () {
            ...
            let url = $.stringFormat("{0}/frPrivatePackage/addOrUpdate", $.cookie('url')),that = this;
                ...
                action: function() {
                  ...
                  axios.get(url, {params:{id:id}})
                    .then(function (res) {
                      let resData = eval(res);
                      if(res.data.code != 500){
                        that.initPrivateCource({curPage:that.privateCourceData.pageNo,limit:that.privateCourceData.pageSize})
                      }
                      $.alert(resData['data']['msg']);
                    }).catch(function (error) {
                    //隐藏加载中
