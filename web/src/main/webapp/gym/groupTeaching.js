    ...
    new Vue({
        ...
        methods:{
          ...
          selectDefault:function () {
            ...
            if(item){
              this.groupCourseData.trainSeriesIds = item.id;
              $('#defaultCurriculumPlanModal').modal('hide');
              $("#privatemrkcjh").val(item.name);
            }else{
              $.alert("请选中训练课程");
              return;
            }
          ...
          deepcopy:function (obj) {
            ...
            saveGroupCourse:function () {
                ...
                if(!this.isNoEmpty(this.groupCourseData.promtionPrice)){
                    errMsg += "促销价必填<br/>"
                }
                ...
                if(!this.isNoEmpty(this.groupCourseData.leastClassNum)){
                  ...
                  if(!this.isNoEmpty(this.groupCourseData.trainSeriesIds)){
                    errMsg += "训练课程筛选必填<br/>"
                  }
                ...
                console.info(this.groupCourseData)
                let url = $.stringFormat("{0}/frGroupCourse/saveOrUpdate", $.cookie('url')),that = this;
...
//						修改 插件527-529行
                url = $.stringFormat("{0}/frGroupSeries/tree",$.cookie('url'));
                Loading.prototype.show();
                axios.get(url,{params: {}})
                    .then(function (res) {
                        if(res.data.code == 200){
