...
Vue.component('edu-pay-bill-children', {//模版挂载的标签名
    ...
    watch: {
        ...
        'memberInfo.ticketList': {//计算数组票据列表的变化
            handler: function (value, oldVal) {
                ...
                //差价添加进小计 原价 --》 + this.memberInfo.cardInfo.selectTotalPrice
    ...
    created() {
        ...
        //     //courseMemberPrice
        // })
        Event.$on(EDU_CONSTANT.listenerChooseClientInfo, (status, eduItem)=>{
            this.isShowBill = status;
            console.log(eduItem)
            this.eduItem = eduItem;
            this.eduItem.payPrice = eduItem.totalPrice - eduItem.noPrice + parseFloat(eduItem.cashPledge);
            this.payMoney.totalPrice = this.eduItem.noPrice ? this.eduItem.noPrice : 0;
            this.payMoney.oldTotalPrice = this.payMoney.totalPrice;
            // //当前总金额
            this.rebate.currentTotalMoney = this.payMoney.totalPrice;
            //获取用户
            axiosGetParams(COURSE_URL.fetchUserInfo, {id: $.cookie('cid')}, (response) => {
                //  debugger
    ...
    methods: {
        ...
        searchByTelAndNo(num){
            ...
            axiosGetParams(COURSE_URL.getClientListByMobile, param, (response) => {
                this.memberInfo.optionClientList = response.data;//导入值
        ...
        findTicketList(mobile, shopId){
            ...
            //console.log(param)
            //ticketListGeneral
            axiosGetParams(COURSE_URL.findPostTicketList, param, (res) =>{
                //  console.log(res)
                let result = res.data;
                let obj = {};
                if(this.index == 0){
        ...
        payBill(){
            ...
            let moneyReport = {
                ...
                sdadiumId: SYSTEM_SETTING,
                Name: this.eduItem.clientName,
                Phone: this.eduItem.Phone
            }
