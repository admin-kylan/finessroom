...
$(document).on('click','.tableList tbody td:nth-child(4)', function () {
    ...
    var url = $.stringFormat('{0}/frLatenceFollow/getPricList','http://www.4006337366.com:8080/');
    $.get(url,{
...
$(document).on('click','table tbody td:nth-child(13)', function () {
    ...
    $.get(url,{
        ...
        if(res.code=='200'){
            ...
            if( _type == 1 ){
                LeadCustomerFollowUpRecord.supperTitle = '修改';
            }
            LeadCustomerFollowUpRecord.supperManagmentName = $.cookie('name');
            LeadCustomerFollowUpRecord.follwAdivce.supperManagmentId = $.cookie('id');
            LeadCustomerFollowUpRecord.follwAdivce.id = followUpId;
            LeadCustomerFollowUpRecord.follwAdivce.checkTime = getNowTime(true);
...
function closeBox2(){
    ...
    LeadCustomerFollowUpRecord.supperTitle = '审核';
    LeadCustomerFollowUpRecord.supperManagmentName='无';
    LeadCustomerFollowUpRecord.follwAdivce.supperManagmentId='';
    LeadCustomerFollowUpRecord.follwAdivce.id='';
    LeadCustomerFollowUpRecord.follwAdivce.handleAdvice='';
};
...
var LeadCustomerFollowUpRecord = new Vue({
    el:'#LeadCustomerFollowUpRecord',
    data:{
    ...
    methods: {
        ...
        getFollowUpRecordList:function (page,limit,id) {
            var that = this;
            var url = $.stringFormat('{0}/frLatenceFollow/getLatenceClientList','http://www.4006337366.com:8080/');
        ...
        toSaveSubmit:function () {
            ...
            var url = $.stringFormat('{0}/frLatenceFollow/toSaveSubmit','http://www.4006337366.com:8080/');
            $.ajax({
                type: 'POST',
                url: url,
                data: data,
        ...
        addAndUpdateLoad: function () {
           ...
           if(!followUpRecordAdd.nextFollowTime){
            ...
            if(isFlage){
                ...
                var url = $.stringFormat('{0}/frLatenceFollow/toAddUpdateLoad','http://www.4006337366.com:8080/');
                $.ajax({
                    type: 'POST',
                    url: url,
                    data: param,
