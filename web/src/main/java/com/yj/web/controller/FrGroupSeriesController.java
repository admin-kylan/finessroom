...
public class FrGroupSeriesController {
    ...
    @GetMapping("/tree")
    public JsonResult getAllSeries(HttpServletRequest request){
        String code = CookieUtils.getCookieValue(request, "code", true);
        List<FrGroupSeries> parent = iFrGroupSeriesService.selectList(new EntityWrapper<FrGroupSeries>().where("(parent_id is null or parent_id='0') and customer_code={0} and is_using=1",code).orderBy("create_time"));
