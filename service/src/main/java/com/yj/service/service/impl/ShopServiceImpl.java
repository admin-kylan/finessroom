...
public class ShopServiceImpl extends BaseServiceImpl<ShopMapper, Shop> implements IShopService {
     ...
     */
    @Override
    public JsonResult getStoreList(Integer type, String code, String shopId) {
        ...
        if (shopId != null && shopId != "") {
            Shop shop = selectOne(
                    new EntityWrapper<Shop>().where("CustomerCode = {0} and id={1}", code, shopId).orderBy("CityId")
            );
            boolean b = true;
            if (id.size() < 1) {
            ...
            for (int i = 0; i < id.size(); i++) {
                List<Shop> list1 = baseMapper.selectList(
                        new EntityWrapper<Shop>().where("CityId={0} and id={1}", id.get(i), shop.getId()).and("CustomerCode = {0}", code)
                );
                list2.add(list1);
            }
...
//        if (shopId == null) {
//            //查全部
//            List<Shop> shops = selectList(
//                    new EntityWrapper<Shop>().setSqlSelect("ID,ShopName").where("Status=0 and CustomerCode={0}", code)
//            );
//            for (Shop shop : shops) {
//                if (shop != null) {
//                    List<Map<String, Object>> maps = sdaduimMapper.selectMaps(new EntityWrapper<Sdaduim>().setSqlSelect("ID,Name").
//                            where("ShopId = {0}", shop.getId()));
