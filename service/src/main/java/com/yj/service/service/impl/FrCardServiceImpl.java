...
//    private FrStoreMapper storeMapper;//门店
    ...
    public JsonResult getMemberCard(String id) {
        ...
        for (Shop shop : storeList) {
            if(shop!=null){
                List<Sdaduim> sdaduims = sdaduimMapper.selectList(new EntityWrapper<Sdaduim>().
                        where("ShopId = {0}", shop.getId()));

                for (Sdaduim sdaduim : sdaduims) {
                    if(sdaduim!=null){
                        List< CategoryItemDTO > items = categoryItemMapper.selectItemsBySdaduimId(sdaduim.getId());
                        sdaduim.setCategoryItemlist(items);
                    }

                }
                shop.setSdaduimArrayList(sdaduims);
            }
        }
...
//        }
        frClient2=new FrClient();
        frClient2.setLevelId("b89cf7118c5d8bc0");
        if (frClient2 != null) {
        ...
        return JsonResult.success(frClient1.getId());
    }
