...
public class JsonResult<T> {
    ...
    public static <T> JsonResult<T> parameterError() {
        JsonResult<T> ret = new JsonResult<T>();
...
/*******************************以下为APP接口所用************************************/
    public static <T> JsonResult<T> appSuccess() {
        JsonResult<T> ret = new JsonResult<T>();
        ret.setCode(JsonReturnCode.APP_SUCCESS.getCode());
        ret.setMsg(JsonReturnCode.APP_SUCCESS.getDesc());
        return ret;
    }

    public static <T> JsonResult<T> appSuccess(T data) {
        JsonResult<T> ret = JsonResult.appSuccess();
        ret.setData(data);
        return ret;
    }

    public static <T>  JsonResult<T> appSuccessMessage(String msg) {
        JsonResult<T> ret = JsonResult.appSuccess();
        ret.setMsg(msg);
        return ret;
    }
